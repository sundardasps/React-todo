...
export const createTodo = async (req, res) => {
  try {
    ...
    const existTask = await todoModel.findOne({title:title})
     if(existTask){
       return res.status(200).json({existTask,message:"Task already in pending."})
     }else{
      ...
      if(saved){
         return res.status(200).json({todoCreated:true,message:"New task added."})
      }
     }
